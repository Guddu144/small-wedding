            ...
            currentData.map((item, index) => (
              ...
              >
                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap hover:underline">
                 <Link href={`/dashboard/venue/view/${item?.venueId}`}>{item?.venue_name}</Link>
                </td>
                <td className="px-6 py-4">{item?.description}</td>
                <td className="px-6 py-4">{item?.venueId}
                {item?.gallery ?
                  <img className="w-20 h-20" src={`${item?.gallery[0] ? `${item?.gallery[0]}`: "https://www.theestateyountville.com/wp-content/uploads/2023/09/The-Estate-Yountville-Partner-Assets-DJI_0045-1_R-820x460.jpg"}`} alt="dashboard"/>
                  :''}
                </td>
